package com.hechang.codeagent.langgraph4j;

import cn.hutool.json.JSONUtil;
import com.hechang.codeagent.exception.BusinessException;
import com.hechang.codeagent.exception.ErrorCode;
import com.hechang.codeagent.langgraph4j.model.QualityResult;
import com.hechang.codeagent.langgraph4j.node.*;
...
public class CodeGenWorkflow {
    ...
    public WorkflowContext executeWorkflow(String originalPrompt) {
        ...
        for (NodeOutput<MessagesState<String>> step : workflow.stream(
            ...
            log.info("--- executeWorkflow 第 {} 步完成 ---", stepCounter);
            // 显示当前状态
            WorkflowContext currentContext = WorkflowContext.getContext(step.state());
            if (currentContext != null) {
                finalContext = currentContext;
                log.info("executeWorkflow 当前步骤上下文: {}", currentContext);
            }
            stepCounter++;
        }
        log.info("executeWorkflow 代码生成工作流执行完成！");
        return finalContext;
    }
        ...
        return Flux.create(sink -> {
            Thread.startVirtualThread(() -> {
                try {
                    ...
                    GraphRepresentation graph = workflow.getGraph(GraphRepresentation.Type.MERMAID);
                    log.info("executeWorkflowWithFlux 工作流图:\n{}", graph.content());

                    int stepCounter = 1;
                    for (NodeOutput<MessagesState<String>> step : workflow.stream(
                            Map.of(WorkflowContext.WORKFLOW_CONTEXT_KEY, initialContext))) {
                        log.info("--- executeWorkflowWithFlux 第 {} 步完成 ---", stepCounter);
                        WorkflowContext currentContext = WorkflowContext.getContext(step.state());
                        if (currentContext != null) {
                            ...
                            log.info("executeWorkflowWithFlux 当前步骤上下文: {}", currentContext);
                        }
                        stepCounter++;
                    }
                    ...
                    log.info("executeWorkflowWithFlux 代码生成工作流执行完成！");
                    sink.complete();
                } catch (Exception e) {
                    log.error("executeWorkflowWithFlux 工作流执行失败: {}", e.getMessage(), e);
